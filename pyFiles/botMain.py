...
async def send_message(message, user_message, is_private):
    try:
        ...
        if (response[1] == 'm'):
            for i in response[0]:
                if '.mp4' in i or '.pdf' in i or '.jpg' in i:
                    await message.author.send("This may take a moment...") if is_private else await message.channel.send("This may take a moment...")
...
def run_discordbot():
    ...
    async def on_message(message):
        ...
        if (user_message[0] == '<@1139830899685474395>' or user_message[0] == '<@&1140177528657551383>' or "PhyXit" == message.mentions[0].name):
            if user_message[1][0] == '$':
